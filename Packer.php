...
class Packer
{
    ...
    public function pack()
    {
        ...
        $this->logger->log(
            LogLevel::INFO, 
            "packing completed, {$packedBoxes->count()} boxes"
        );
        return $packedBoxes;
    }
    ...
    public function redistributeWeight(PackedBoxList $aPackedBoxes)
    {
        ...
        $this->logger->log(
            LogLevel::DEBUG,  
            "repacking for weight distribution, weight variance "
            . "{$aPackedBoxes->getWeightVariance()}, target weight {$targetWeight}"
        );
        ...
        do { //Keep moving items from most overweight box to most underweight box
            ...
            $this->logger->log(
                LogLevel::DEBUG,  
                'boxes under/over target: ' . count($underWeightBoxes) 
                . '/' . count($overWeightBoxes)
            );
            ...
            foreach ($underWeightBoxes as $u => $underWeightBox) {
                foreach ($overWeightBoxes as $o => $overWeightBox) {
                    ...
                    // For each item in the heavier box, try and move 
                    // it to the lighter one
                    foreach ($overWeightBoxItems as $oi => $overWeightBoxItem) {
                        if ($underWeightBox->getWeight() 
                            + $overWeightBoxItem->getWeight() > $targetWeight
                        ) {
                            // skip if moving this item would hinder 
                            // rather than help weight distribution
                            continue; 
                        }
    ...
    public function packIntoBox(Box $aBox, ItemList $aItems)
    {
        $this->logger->log(
            LogLevel::DEBUG,  
            "[EVALUATING BOX] {$aBox->getReference()}"
        );
